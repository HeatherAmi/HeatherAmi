# Hi, I'm Heather

I am a Microsoft Certified Trainer specialising in the Power Platform, working under the banner of **Heather Ami Digital**.

I build bite-size, follow-along projects to help others learn the Power Platform, prepare for Microsoft certifications, and pass their applied skills assessments. Everything I publish here is designed to be practical, repeatable, and shared freely with the community.

## What you will find on this profile

### Certifications
Projects, guides, and walkthroughs aligned to specific Microsoft certifications.

- [PL-400: Power Platform Developer](https://github.com/HeatherAmi/certifications-pl400)
- [PL-200: Power Platform Functional Consultant](https://github.com/HeatherAmi/certifications-pl200)
- [PL-900: Power Platform Fundamentals](https://github.com/HeatherAmi/certifications-pl900)
- [AZ-900: Microsoft Azure Fundamentals](https://github.com/HeatherAmi/certifications-az900)
- [Applied Skills assessments](https://github.com/HeatherAmi/certifications-applied-skills)


### Foundations
One-time setup content that the certification and training repos build on top of.

- [Foundation Setup: Tenant, Entra ID, Environments, Tooling](https://github.com/HeatherAmi/foundation-setup)

### Training Days
Full-day workshop content for live and recorded delivery.

- [Training Days repo](https://github.com/HeatherAmi/training-days)
  - Dataverse in a Day
  - Canvas Apps in a Day
  - Model-Driven Apps in a Day
  - Power Automate in a Day
  - Security in a Day


## YouTube

Every project in these repos has a matching video walkthrough on my YouTube channel. Videos are kept to around 15 minutes each, so you can follow along in short, focused sessions.

[youtube.com/@HeatherAmiDigital](https://www.youtube.com/@HeatherAmiDigital)

## Get in touch

If you spot anything that could be improved, or you have a request for a future project, please open an issue on the relevant repo or reach out on LinkedIn.

[linkedin.com/in/heather-ami-digital](https://www.linkedin.com/in/heather-ami-digital/)
